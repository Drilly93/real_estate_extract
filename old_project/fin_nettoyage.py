...
COLUMN_DROP_FOR_DATA_SET = ['date_mutation','nature_mutation',
                            'id_parcelle','prix_par_m2_habitable',
                            'prix_par_m2_terrain']
...
def nettoyer_valeur_fonciere(df, seuil_min=None, seuil_max_quantile=0.999, seuil_min_quantile=0.001):
    ...
    df = df.drop(COLUMN_DROP_FOR_DATA_SET)
    df.write_parquet(PATH_DIR_DF_FINAL / "data_set.parquet") # On enregistre le dataset et on enleve les donnees inutiles
